...
Also set a few higher level global variables
...
- Deposit the phenotype parquet file in `parsed/phenotype/phenotype.parquet`
...
python3 ${AMR_SCRIPTS}/convert_and_merge_csv_to_parquet.py \
...
${AMR_SCRIPTS}/add_country_from_country_code.py \
...
- Drop the `antibiotic_abbreviation` column
...
for file in $workdir/*.parquet; do
    ...
    ${AMR_SCRIPTS}/parquet_to_csv_gz.py $file ${file%.parquet}.csv.gz
done
```
